...
export class PaymentClient {
  ...
  async createPayment(input: CreatePaymentDTO, receiver: UUID, bearerToken: string, ): Promise<UUID> {
    const mutation = gql`
    ...
    try {
      ...
      const data: any = await request(this.#graphqlEndpoint, mutation, { input, receiver }, headers);
